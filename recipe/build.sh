...
# -c opt: bazel defaults to fastbuild (unoptimized); optimize like upstream's
# own wheels (fastbuild made profiler_plugin_c_api.so ~220 MB and slow on the
...
# strip the native library (bazel doesn't strip the cc_shared_library output);
# keep dynamic symbols, drop debug + local — brings the ~65 MB opt .so toward
# upstream's ~40 MB stripped wheel size
if [[ "$(uname)" == "Darwin" ]]; then STRIP_CMD=(strip -x); else STRIP_CMD=("${STRIP:-strip}"); fi
find . -name '*.so' -o -name '*.dylib' | while read -r lib; do "${STRIP_CMD[@]}" "$lib" || true; done
